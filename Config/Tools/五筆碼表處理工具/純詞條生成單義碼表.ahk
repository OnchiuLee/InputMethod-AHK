/*
	純詞組生成帶編碼的單義碼表：
		先選擇純詞條文本，再選擇正常的單義/多義碼表作爲生成編碼依據，碼表格式建議「UTF-8 BOM 或 UTF-16LE BOM」
*/
#NoEnv
#NoTrayIcon
...
FileSelectFile, MaBiaoFile, 3, , 選擇純詞組碼表, Text Documents (*.txt)
...
If (MaBiaoFile<> ""&&filename){
	...
	If not Encoding~="UTF\-16BE\sBOM|UTF\-8\sBOM" {
		MsgBox, 262160, 错误提示, %MaBiaoFile%文件编码格式非〔UTF-8 BOM 或 UTF-16LE BOM 或 CP936〕！, 4
		ExitApp
	}
	If MaBiao~="[^\s\t\w]" {
		FileSelectFile, dzMaBiaoFile, 3, , 导入單字码表, Text Documents (*.txt)
		If (dzMaBiaoFile<>""){
			GetFileFormat(dzMaBiaoFile,MaBiao_dz,Encoding)
			If not Encoding~="UTF\-16BE\sBOM|UTF\-8\sBOM" {
				MsgBox, 262160, 错误提示, %dzMaBiaoFile%文件编码格式非〔UTF-8 BOM 或 UTF-16LE BOM 或 CP936〕！, 4
				ExitApp
			}
			if MaBiao_dz~="`n[a-z]\s.+\s.+" {
				MaBiao_dz:=TransformCiku(MaBiao_dz)
			}
			If objCount(EN_Phrase:=GetCikuFullcode(MaBiao_dz)) {
				...
				Loop,Parse,MaBiao,`n,`r
				{
					If (A_LoopField) {
						...
						tarr:=A_LoopField
						If (StrLen(tarr)=1)
							bianma:=EN_Phrase[tarr,1]
						If (StrLen(tarr)=2)
							bianma:=EN_Phrase[SubStr(tarr,1,1),2] EN_Phrase[SubStr(tarr,1,1),3] EN_Phrase[SubStr(tarr,2,1),2] EN_Phrase[SubStr(tarr,2,1),3]
						If (StrLen(tarr)=3)
							bianma:=EN_Phrase[SubStr(tarr,1,1),2] EN_Phrase[SubStr(tarr,2,1),2] EN_Phrase[SubStr(tarr,3,1),2] EN_Phrase[SubStr(tarr,3,1),3]
						else If (StrLen(tarr)>3)
							bianma:=EN_Phrase[SubStr(tarr,1,1),2] EN_Phrase[SubStr(tarr,2,1),2] EN_Phrase[SubStr(tarr,3,1),2] EN_Phrase[SubStr(tarr,0),2]

						result.=tarr "`t" bianma "`r`n", bianma:=""
						If (Mod(count, num)=0) {
							tx :=Ceil(count/num)
				...
				If (result) {
					FileDelete,%FileFoldPath%%FileName%_Ci.txt
					FileAppend,%result%,%FileFoldPath%%FileName%_Ci.txt,utf-8
				}
			}else
				MsgBox, 262160, 错误提示, % EN_Phrase?EN_Phrase:"生成失敗！", 8
		}
	}else
