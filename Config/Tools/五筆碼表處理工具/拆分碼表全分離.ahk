/*
	僅支持单行單义，分離拆分單字碼表的每個字根，導出爲單義碼錶，支持單字節與雙字節字根混搭碼表，碼表格式建議「UTF-8 BOM 或 UTF-16LE BOM」
	支持格式：
		詞條+tab+拆分+編碼----------->詞條+tab+編碼首位編碼+tab+拆分第二位編碼+tab+拆分第三位編碼+tab+拆分末位編碼
		詞條+tab+拆分----------->詞條+tab+拆分首位字根+tab+拆分次第二位字根+tab+拆分第三位字根+tab+拆分末位字根
*/
#NoEnv
#NoTrayIcon
...
If (MaBiaoFile<> ""&&filename){
	MsgBox, 262452, 提示, 是否分離拆分码表？`n词库格式须为「單字+tab+拆分」或「單字+拆分+編碼」
	IfMsgBox, Yes
	{
		...
		If MaBiao~="\n.+\t.+"
		{
			...
			Loop,Parse,MaBiao,`n,`r
			{
				If (A_LoopField) {
					...
					str2:=count_1>0?(tarr[2]~=chr(tarr2[3])?chr(tarr2[3]):chr(tarr2[3]) chr(tarr2[4]), count_1++):(tarr[2]~=chr(tarr2[2])?chr(tarr2[2]):chr(tarr2[2]) chr(tarr2[3]),count++)
					If (count_1>1) {
						str3:=tarr[2]~=chr(tarr2[5])?chr(tarr2[5]):(chr(tarr2[5]) chr(tarr2[6]), count_1++)
					}else If (count_1=1) {
						str3:=tarr[2]~=chr(tarr2[4])?chr(tarr2[4]):(chr(tarr2[4]) chr(tarr2[5]), count_1++)
					}else{
						str3:=tarr[2]~=chr(tarr2[3])?chr(tarr2[3]):(chr(tarr2[3]) chr(tarr2[4]), count_1++)
					}
					If (count_1>2) {
						str4:=tarr[2]~=chr(tarr2[7])?chr(tarr2[7]):(chr(tarr2[7]) chr(tarr2[8]), count_1++)
					}else If (count_1=2) {
						str4:=tarr[2]~=chr(tarr2[6])?chr(tarr2[6]):(chr(tarr2[6]) chr(tarr2[7]), count_1++)
					}else If (count_1=1) {
						str4:=tarr[2]~=chr(tarr2[5])?chr(tarr2[5]):(chr(tarr2[5]) chr(tarr2[6]), count_1++)
					}else{
						str4:=tarr[2]~=chr(tarr2[4])?chr(tarr2[4]):(chr(tarr2[4]) chr(tarr2[5]), count_1++)
					}
					InsertArr1.=tarr[1] "`t" str1 "`t" str2 (str3?"`t" str3:"") (str4?"`t" str4:"") "`r`n", str1:=str2:=str3:=str4:=""
					If (objLength(tarr)>2&&tarr[3]~="[a-z]")
						InsertArr2.=tarr[1] "`t" SubStr(tarr[3],1,1) "`t" SubStr(tarr[3],2,1) (SubStr(tarr[3],3,1)?"`t" SubStr(tarr[3],3,1):"") (SubStr(tarr[3],4,1)?"`t" SubStr(tarr[3],4,1):"") "`r`n"
					If (Mod(count, num)=0) {
						tx :=Ceil(count/num)
			...
			If InsertArr1
			{
				...
				If (MaBiao~="\t[a-z]") {
					FileDelete,%FileFoldPath%BianmaPhrase.txt
					FileAppend,% RegExReplace(InsertArr2,"^\n|\n$"),%FileFoldPath%BianmaPhrase.txt,UTF-8
				}
