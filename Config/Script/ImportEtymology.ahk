/*
	拆分词库包含两个：
		一个拆分单字全码词库、一个首字根+次字根词库（脚本从拆分全码码表自动处理）
		选择前处理好拆分单字词库，不要出现重复。非98五笔的选择font目录下的wubizg.otf字体，非五笔的自己去主脚本把你的拆分字体内部名称加入过滤FontExtend变量
*/
#NoEnv
#NoTrayIcon
...
FileSelectFile, MaBiaoFile, 3, , 请选择拆分單字码表, Text Documents (*.txt)
...
If (MaBiaoFile<> ""&&filename){
	MsgBox, 262452, 提示, 是否导入拆分单字源码表？`n词库格式须为「單字+tab+拆分」
	IfMsgBox, Yes
	{
		...
		If not Encoding~="UTF\-16BE\sBOM|UTF\-8\sBOM" {
			MsgBox, 262160, 错误提示, 文件编码格式非〔UTF-8 BOM 或 UTF-16LE BOM〕！, 10
			ExitApp
		}
		If MaBiao~="\n\W+\t\W+$"
		{
			...
			Loop,Parse,MaBiao,`n,`r
			{
				If (A_LoopField) {
					count++, count_1:=0
					tarr:= StrSplit(A_LoopField,A_Tab), tarr2:=StrSplit(Char2Unicode(RegExReplace(tarr[2],"\〔|\〕|\「|\」|\（|\）")),"|")
					...
					InsertArr1 .="('" tarr[1] "','" str1 "','" str2 "')" ","
					InsertArr2 .="('" tarr[1] "','" ("〔" RegExReplace(tarr[2],"\〔|\〕|\「|\」|\（|\）") "〕") "')" ",", str1:=str2:=""
					If (Mod(count, num)=0) {
						tx :=Ceil(count/num)
			...
			Progress,off
			If (InsertArr1&&InsertArr2)
			{
				MsgBox, 262452, 写入提示, 码表处理完成是否写入到词库？`n写入需要几秒钟，请等待。。。
				IfMsgBox, Yes
				{
					Progress, M ZH-1 ZW-1 Y100 FM12 C0 FM14 WS700 ,, EtymologyChr词库导入中！, 导入提示
					Create_EtymologyChr(DB)
					If DB.Exec("INSERT INTO EtymologyChr VALUES " RegExReplace(InsertArr2,"\,$","") ";")>0
					{
						Progress, M ZH-1 ZW-1 Y100 FM12 C0 FM14 WS700 ,, EtymologyPhrase词库导入中！, 导入提示
						Create_EtymologyPhrase(DB)
						If DB.Exec("INSERT INTO EtymologyPhrase VALUES " RegExReplace(InsertArr1,"\,$","") ";")>0
						{
							...
							MsgBox, 262160, 错误提示, EtymologyPhrase词库导入失敗！, 5
					}else
						MsgBox, 262160, 错误提示, EtymologyChr词库导入失敗！, 5

				}
				MaBiao:=InsertArr2:=InsertArr1:="",tarr:=tarr2:=[], mbObj:={}
			}else{
				MsgBox, 262160, 错误提示, 處理失敗！, 5
			}
		}else{
			MsgBox, 262160, 错误提示, 码表格式不对！, 5
			ExitApp
		}
		DB.CloseDB()
	}
...
Create_EtymologyChr(DB)
{
	DB.Exec("DROP TABLE IF EXISTS EtymologyChr;")
	DB.Exec("BEGIN TRANSACTION;")
	DB.Exec("CREATE TABLE EtymologyChr ('aim_chars' TEXT,'A_Key' TEXT );")
	DB.Exec("CREATE INDEX IF NOT EXISTS 'main'.'sy_EtymologyChr' ON 'EtymologyChr' ('aim_chars');")
	DB.Exec("COMMIT TRANSACTION;VACUUM;")
}

Create_EtymologyPhrase(DB)
{
	DB.Exec("DROP TABLE IF EXISTS EtymologyPhrase;")
	DB.Exec("BEGIN TRANSACTION;")
	DB.Exec("CREATE TABLE EtymologyPhrase ('aim_chars' TEXT,'A_Key' TEXT,'B_Key' TEXT );")
	DB.Exec("CREATE INDEX IF NOT EXISTS 'main'.'sy_EtymologyPhrase' ON 'EtymologyPhrase' ('aim_chars');")
	DB.Exec("COMMIT TRANSACTION;VACUUM;")
}
